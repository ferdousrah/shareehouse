...
class RsExplodinglayersSliderFront extends RevSliderFunctions {
	...
	private function isEnabled($slider){
		...
		foreach($layers ?? [] as $layer){
			...
			foreach($frames ?? [] as $frame){
				if($this->get_val($frame, array('explodinglayers', 'use'), false) === true) return true;
			}
		}
	...
	public function add_scripts(){
		...
		$_jsPathMin = file_exists(RS_EXPLODINGLAYERS_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['explodinglayers'] = '<script'. $addition .' src="'. $this->pluginUrl . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
		
		return $return;
	}
	
	public function add_waiting_script_slugs($wait){
		$wait[] = 'explodinglayers';
		return $wait;
	}
	...
	public function write_init_script($slider, $id){
		...
		if(!empty($enabled)){
			...
			echo "\n";
			echo '						if(typeof ExplodingLayersAddOn !== "undefined") ExplodingLayersAddOn(tpj, revapi' . $id . ');';
