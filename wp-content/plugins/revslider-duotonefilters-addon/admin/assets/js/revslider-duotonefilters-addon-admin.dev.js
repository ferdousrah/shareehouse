/***************************************************
 * REVOLUTION 6.7.0 DUOTONE ADDON
...
(function() {
		...
		jQuery(document).on(slug+'_init',function() {			
			if (RVS.SBGS===undefined) {
				setTimeout(function() {
					jQuery(document).trigger(slug+'_init');
				},200);
				return;
			}
		...
		function initInputs() {
			...
			// init ddTP slide
			addon.selectFilter = addon.forms.slidegeneral.find('.tos2.nosearchbox').ddTP({
		...
		var events = {
			...
			updateDuotone: function() {
				...
				RVS.SBGS[RVS.S.slideId].wrap.dataset.duotone = 'rs-duotone-' + filter;
			...
			*/
			duotoneHover: function(e) {
				...
				if(e.type === 'duotoneEnter') {
					...
					if(defFilter !== 'none') RVS.SBGS[RVS.S.slideId].wrap.className = '';
				...
				// mouseleave
					...
					if(duotone !== 'none') {
						...
						// draw filter onto the stage
						addon.body.addClass('duotone_active');
						RVS.SBGS[RVS.S.slideId].wrap.dataset.duotone = 'rs-duotone-' + duotone;						
					...
					// restore default filter
						...
						addon.body.removeClass('duotone_active');
						if(defFilter !== 'none') RVS.SBGS[RVS.S.slideId].wrap.classList.add(defFilter);
