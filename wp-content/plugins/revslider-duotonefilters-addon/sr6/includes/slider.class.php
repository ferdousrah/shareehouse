...
class RsDuotoneSliderFront extends RevSliderFunctions {
	...
	public function add_addon_files($html, $export){
		...
		$_jsPathMin = file_exists(RS_DUOTONE_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
		if(!$export->usepcl){
			$export->zip->addFile(RS_DUOTONE_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', 'js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js');
			$export->zip->addFile(RS_DUOTONE_PLUGIN_PATH . 'sr6/assets/css/revolution.addon.' . $this->pluginTitle . '.css', 'css/revolution.addon.' . $this->pluginTitle . '.css');
		}else{
			$export->pclzip->add(RS_DUOTONE_PLUGIN_PATH.'sr6/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', PCLZIP_OPT_REMOVE_PATH, RS_DUOTONE_PLUGIN_PATH.'sr6/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
			$export->pclzip->add(RS_DUOTONE_PLUGIN_PATH.'sr6/assets/css/revolution.addon.' . $this->pluginTitle . '.css', PCLZIP_OPT_REMOVE_PATH, RS_DUOTONE_PLUGIN_PATH.'sr6/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');
		}

		$html = str_replace(RS_PLUGIN_URL.'sr6/assets/fonts/font-awesome/css/font-awesome.css', 'css/font-awesome.css', $html);
	...
	private function isEnabled($slider){
		...
		if($static_slide instanceof RevSliderSlide){
			...
			if($this->get_val($params, array('addOns', 'revslider-' . $this->pluginTitle . '-addon', 'filter'), 'none') !== 'none') return true;
		}
		...
		foreach($slides ?? [] as $slide){
			if($this->get_val($slide, array('params', 'addOns', 'revslider-' . $this->pluginTitle . '-addon', 'filter'), 'none') !== 'none') return true;
		}
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['duotone'] = '<script'. $addition .' src="'. $this->pluginUrl . 'sr6/assets/js/revolution.addon.' . $this->pluginTitle  . $_jsPathMin . '.js"></script>';
