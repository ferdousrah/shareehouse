...
class SrDuotoneFiltersFront extends RevSliderFunctions {
	...
	public function get_script_list(){
		$min = file_exists(RS_DUOTONE_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';
		return array(
			'duotonefilters' => RS_DUOTONE_PLUGIN_URL.'public/js/' . $this->slug . $min . '.js',
			'cssduotonefilters' => RS_DUOTONE_PLUGIN_URL.'public/css/' . $this->slug . '.css'
		);
	}
	...
	public function enqueue_scripts(){
		...
		$this->script_enqueued = true;		
		$min = file_exists(RS_DUOTONE_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';
		wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_DUOTONE_PLUGIN_URL . "public/js/" . $this->slug . $min . ".js", '', RS_REVISION, array('strategy' => 'async'));
		wp_enqueue_style('revslider-'.$this->slug.'-addon-css', RS_DUOTONE_PLUGIN_URL . "public/css/" . $this->slug . ".css", '', RS_REVISION);
	}
	...
	public function add_addon_files($html, $export){
		...
		foreach($list ?? [] as $handle => $script){
			...
			$script_path = str_replace(RS_DUOTONE_PLUGIN_URL, RS_DUOTONE_PLUGIN_PATH, $script);
			if(!$export->usepcl){
				$export->zip->addFile($script_path,  $export_path . $handle . '.'.$extension);
			}else{
				$export->pclzip->add($script_path, PCLZIP_OPT_REMOVE_PATH, RS_DUOTONE_PLUGIN_PATH.'public/'.$extension.'/', PCLZIP_OPT_ADD_PATH, $export_path);
			}
