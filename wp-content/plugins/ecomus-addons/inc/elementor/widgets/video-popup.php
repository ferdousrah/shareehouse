...
class Video_Popup extends Widget_Base {
		...
		$this->add_group_control(
			...
			[
				...
				'fields_options'  => [
					...
					'image' => [
						'default'   => [
							'url' => 'https://placehold.co/1440x500?text=Video%20Thumbnail',
						],
					],
				],
				'separator' => 'before',
	...
	protected function render() {
		...
		$aria_label = esc_html__( 'Play video', 'ecomus-addons' );

		$btn_full = '';
		if ( $video_url['url']) :
			if ( $settings['link_type'] == 'only') {
				$marker_html =  '<a href="'. ( $video_url['url'] ) .'" class="ecomus-video-popup__play" aria-label="'. esc_attr( $aria_label ) .'">'. $marker_html .'</a>';
			} else {
				$btn_full =  '<a href="'. esc_url( $video_url['url'] ) .'" class="ecomus-video-popup__play full-box-button" aria-label="'. esc_attr( $aria_label ) .'"></a>';
			}
		endif;
