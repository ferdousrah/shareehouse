...
class Store_Locations extends Widget_Base {
	...
	protected function section_content() {
		...
		$repeater->add_control(
			'website', [
				'label' => esc_html__( 'Website', 'ecomus-addons' ),
	...
	protected function render() {
        ...
        foreach ( $settings['locations'] as $location ) :
            ...
            $website   = ! empty( $location['website'] ) ? '<div '. $this->get_render_attribute_string( 'info' ) .'><span '. $this->get_render_attribute_string( 'span' ) .'>'. esc_html__( 'Website', 'ecomus-addons' ) .'</span><div>'. wp_kses_post( nl2br( $location['website'] ) ) .'</div></div>' : '';
