...
class Accordion extends Widget_Base {
	...
	protected function schema_render() {
		$settings = $this->get_settings_for_display();
		...
		if ( ! empty( $settings['tabs'] ) ) {
			...
			foreach ( $settings['tabs'] as $tab ) {
				...
				$answer   = wp_kses_post( $tab['tab_content'] );
			...
			if ( ! empty( $faq_data['mainEntity'] ) ) {
				echo '<script type="application/ld+json">' . wp_json_encode( $faq_data, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT ) . '</script>';
			}
		}
	}
}
