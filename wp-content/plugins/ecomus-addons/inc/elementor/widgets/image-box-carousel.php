...
class Image_Box_Carousel extends Carousel_Widget_Base {
	...
	protected function section_content_slides() {
		...
		$this->add_control(
			'heading_size',
			[
				'label' => __( 'Heading HTML Tag', 'ecomus-addons' ),
        ...
        $repeater->add_responsive_control(
			...
			[
				...
				'default' => [
					'url' => 'https://placehold.co/428x547?text=image',
				],
			]
		);
	...
	protected function section_view_all_button_options() {
        ...
        $this->add_responsive_control(
			...
			[
				...
				'type' => Controls_Manager::MEDIA,
				'default' => [
					'url' => 'https://placehold.co/130x130?text=image',
				],
                'condition' => [
        ...
        $this->add_responsive_control(
			...
			[
				...
				'selectors'            => [
					...
					'{{WRAPPER}} .ecomus-image-box-carousel__content' => 'align-items: {{VALUE}}',
					'{{WRAPPER}} .ecomus-image-box-carousel__content .ecomus-image-box-carousel__button' => 'align-self: {{VALUE}}',
				],
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'            => [
					...
					'{{WRAPPER}} .ecomus-image-box-carousel__content' => 'justify-content: {{VALUE}}',
				],
				'selectors_dictionary' => [
					'top'   => 'flex-start',
					'middle' => 'center',
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'   => [
					...
					'{{WRAPPER}} .ecomus-image-box-carousel__align-stretch' => 'text-align: {{VALUE}}',
					'{{WRAPPER}} .ecomus-image-box-carousel__content' => 'text-align: {{VALUE}}',
				],
			]
		);
	...
	protected function render() {
        ...
        $this->add_render_attribute( 'heading', 'class', [ 'ecomus-image-box-carousel__heading', 'ecomus-image-box-carousel__heading--' . $settings['heading_navigation_type'], 'heading--carousel', 'em-flex', 'em-flex-align-center', 'em-relative' ] );
        ...
        $this->add_render_attribute( 'item', 'class', [ 'ecomus-image-box-carousel__item', 'ecomus-image-box-carousel__item-' . $settings['content_position'], 'ecomus-image-box-carousel__align-' . $settings['content_align'], 'em-flex', 'em-relative', 'swiper-slide',  $settings['hover_zoom'] == 'yes' ? 'em-eff-img-zoom' : '' ] );
        $this->add_render_attribute( 'image', 'class', [ 'ecomus-image-box-carousel__image', 'em-ratio', 'em-image-rounded' ] );
		$this->add_render_attribute( 'content', 'class', [ 'ecomus-image-box-carousel__content', 'em-absolute', 'em-flex', 'em-flex-column' ] );
        ...
        <div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			<?php if( $settings['show_heading'] == 'yes' && ! empty( $settings['heading'] ) ) : ?>
				<div <?php echo $this->get_render_attribute_string( 'heading' ); ?>>
					...
					<?php endif; ?>
					<<?php echo $settings['heading_size']; ?> class="heading"><?php echo wp_kses_post( $settings['heading'] ); ?></<?php echo $settings['heading_size']; ?>>
						...
						<?php foreach( $settings['items'] as $index => $item ) : ?>
							...
							<div <?php echo $this->get_render_attribute_string( 'item' ); ?>>
								<?php if( ! empty( $item['image'] ) && ! empty( $item['image']['url'] ) ) : ?>
									...
									?>
								<?php endif; ?>
								<?php if ( $settings['content_align'] == 'bottom' && $settings['content_position'] == 'side' ) : ?>
									<?php if ( ! empty( $item['button_link']['url'] ) ) { ?>
									<a class="ecomus-image-box-button--all" href="<?php echo esc_url( $item['button_link']['url']) ?>">
									<span class="screen-reader-text"><?php echo $item['button_text']; ?></span>
									</a>
									<?php } ?>
									...
									<?php if ( $item['title'] || $item['description'] ) : ?>
										<div <?php echo $this->get_render_attribute_string( 'summary' ); ?>>
											<?php if ( $item['before_title'] ) : ?>
												<div <?php echo $this->get_render_attribute_string( 'before_title' ); ?>>
