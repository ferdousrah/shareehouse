...
class Products_Filter extends Widget_Base {
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			...
			[
				...
				'default' => 'name',
				'options' => array(
					'name' => __( 'Name', 'ecomus-addons' ),
					'count' => __( 'Count', 'ecomus-addons' ),
					'id' => __( 'ID', 'ecomus-addons' ),
					'slug' => __( 'Slug', 'ecomus-addons' ),
   ...
   /**
	...
	public function display_filter( $filter ) {
		...
		<div class="<?php echo esc_attr( join( ' ', $classes ) ) ?>">
			...
			<div class="products-filter__filter-control filter-control">
				<?php
				if ( $filter['searchable'] && ! in_array( $filter['display'], array(  'slider', 'ranges' ) ) && ! in_array( $filter['source'], array( 'stock', 'product_cat', 'rating' ) ) ) {
		...
		switch ( $filter['source'] ) {
			...
			default:
				...
				if ( $taxonomy == 'product_cat' && '0' == $filter['multiple'] && ('list' == $filter['display'] || 'checkboxes' == $filter['display'] ) ) {
					$orderby = isset( $filter['product_tax_orderby'] ) ? $filter['product_tax_orderby'] : 'name';
					$terms = $this->display_subs_categories( $show_children_only, $hide_empty, $orderby );
	 ...
	 * @param array $args
	 */
	protected function display_subs_categories( $show_children_only, $hide_empty, $orderby = 'name' ) {
		$cat_ancestors = 0;
		...
		switch ( $args['type'] ) {
			case 'color':
				...
				foreach ( $args['options'] as $slug => $option ) {
					...
					printf(
						'<span class="products-filter__option swatch swatch-color swatch-%s %s %s" data-value="%s" title="%s" %s><span class="bg-color" style="background-color:%s;"><span class="product-variation-item__color"></span></span>%s%s</span>',
						esc_attr( $slug ),
