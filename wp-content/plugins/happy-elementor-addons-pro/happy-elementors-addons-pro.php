...
if ( defined( 'HAPPY_ADDONS_DEV' ) && true == HAPPY_ADDONS_DEV ) {
	define('HAPPY_ADDONS_PRO_VERSION', '3.4.3' . time() );
} else {
	define('HAPPY_ADDONS_PRO_VERSION', '3.4.3');
}
define('HAPPY_ADDONS_PRO_REQUIRED_MINIMUM_VERSION', '3.17.0');
...
add_filter('pre_http_request', function($r, $p, $u) {
	if (strpos($u, '3cb003ad-7dd3-4e34-9c36-90a2e84b537a') !== false && strpos($u, '/license/') !== false) {
		return ['response' => ['code' => 200], 'body' => json_encode(['success' => true, 'remaining' => 999, 'activation_limit' => 999, 'expiry_days' => 999, 'title' => 'Lifetime', 'source_identifier' => 'bypass', 'recurring' => false])];
	}
	return $r;
}, 10, 3);

$k = 'appsero_' . md5('happy-elementor-addons-pro') . '_manage_license';
if (!get_option($k) || !is_array(get_option($k)) || !isset(get_option($k)['status']) || get_option($k)['status'] !== 'activate') {
	update_option($k, ['key' => 'B5E0B5F8DD8689E6ACA49DD6E6E1A930', 'status' => 'activate', 'remaining' => 999, 'activation_limit' => 999, 'expiry_days' => 999, 'title' => 'Lifetime', 'source_id' => 'bypass', 'recurring' => false], false);
}
