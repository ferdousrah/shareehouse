...
class Ajax_Handler {
	...
	public function init() {
		...
		add_action('wp_ajax_hapro_loop_grid_action', [$this, 'loop_grid_ajax']);
		add_action('wp_ajax_nopriv_hapro_loop_grid_action', [$this, 'loop_grid_ajax']);
	...
	/**
	 * Loop Grid Ajax Handler
	 */
	public function loop_grid_ajax() {
		...
		if (true == $security && isset($_POST['querySettings'])) :
			...
			$settings = ha_pro_sanitize_array_recursively($_POST['querySettings']);
			$loaded_item = intval($_POST['loadedItem']);
			$template_id = apply_filters('wpml_object_id', intval($settings['template_id']), 'ha_library');
			...
			if ($_query->have_posts()) :
				while ($_query->have_posts()) : $_query->the_post();
					...
					echo ha_elementor()->frontend->get_builder_content_for_display( $template_id );
					echo "</div>";
	...
	public static function instagram_feed_ajax() {
		...
		if (true == $security && isset($_POST['query_settings'])) :
			...
			$ha_insta_access_token = '_ha_insta_access_token_' . $widget_id;
			$access_token = get_transient( $ha_insta_access_token );
...
?>
	...
	public static function facebook_feed_ajax() {
		...
		if (true == $security && isset($_POST['query_settings'])) :
			...
			$ha_facebook_feed_access_token_key = '_ha_facebook_feed_access_token_' . $settings['widget_id'];
			$access_token = get_transient( $ha_facebook_feed_access_token_key );

			$ha_facebook_feed_page_id_key = '_ha_facebook_feed_page_id_' . $settings['widget_id'];
			$page_id = get_transient( $ha_facebook_feed_page_id_key );
...
<?php
	...
	public static function save_happy_menu_item_settings() {
		...
		update_post_meta($menu_item_id, \Happy_Addons_Pro\Classes\Mega_Menu::$menuitem_settings_key, $menu_item_settings);
	...
	public static function get_happy_menu_item_settings() {
		...
		$data = get_post_meta($menu_item_id, \Happy_Addons_Pro\Classes\Mega_Menu::$menuitem_settings_key, true);
